  ...
  return (
    ...
    <div className="sm:flex flex-col items-center xl:items-start xl:w-[340px] p-2 fixed h-full">
      <div className="space-y-2.5  mt-3 mb-2.5 xl:ml-24">
        <FaXTwitter size={28} className="ml-4 mb-2" />
        <SidebarLink link='' text="Inicio" Icon={<RiHome7Fill size={28}/>} />
        <SidebarLink link={username} text="Perfil" Icon={<BsPersonFill size={28}/>} />
        <SidebarLink link='/contacts' text="Chat" Icon={<HiOutlineEnvelope size={28}/>} />
        <SidebarLink link='/noti'text="Notificaciones" num={data?.length} Icon={<IoIosNotificationsOutline size={28}/>} />
          <button 
          className="ml-1 rounded-full text-xl p-4 flex text-slate-200 hover:bg-blue-300 hover:bg-opacity-10 cursor-pointer"
          onClick={logout}><BiLogOutCircle size={28}/>
          </button>
      </div>
    </div>
  )
